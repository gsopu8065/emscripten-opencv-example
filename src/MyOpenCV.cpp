...
vector<Point> rectangleToPoints(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4){
    vector<Point> p;
...
emscripten::val birdEyeView(const int & addr, const size_t width, const size_t height, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4){
    uint8_t * data = reinterpret_cast<uint8_t *>(addr);
    Mat image(height, width, CV_8UC4, data);

    vector<Point> p = rectangleToPoints(x1, y1, x2, y2, x3, y3, x4, y4);

    float AB = distance(p[0], p[1]);
    float BC = distance(p[1], p[2]);
    float CD = distance(p[2], p[3]);
    float DA = distance(p[3], p[0]);

    float maxWidth = (AB > CD)? AB : CD;
    float maxHeight = (BC > DA)? BC : DA;

    cv::Point2f src_p[4];
    cv::Point2f dst_p[4];

    for(int i=0;i<4;i++){
     src_p[i] = cv::Point2f(p[i].x, p[i].y);
    }

    dst_p[0] = cv::Point2f(  0.0f, 0.0f);
    dst_p[1] = cv::Point2f(  maxWidth - 1,   0.0f);
    dst_p[2] = cv::Point2f(  maxWidth - 1, maxHeight - 1);
    dst_p[3] = cv::Point2f(0.0f,   maxHeight - 1);

    cv::Mat trans_mat33 = cv::getPerspectiveTransform(src_p, dst_p);

    Mat dst_img;
    cv::warpPerspective(image, dst_img, trans_mat33, Size(maxWidth, maxHeight), cv::INTER_LINEAR);

    //Mat noise_img;
    //fastNlMeansDenoisingColored(dst_img, noise_img);


    val result = val(emscripten::memory_view<uint8_t>((dst_img.total()*image.elemSize())/sizeof(uint8_t), (uint8_t *)dst_img.data));
    image.release();
    trans_mat33.release();
    dst_img.release();
    return result;
...
EMSCRIPTEN_BINDINGS(my_module) {
  ...
  emscripten::function("birdEyeView", &birdEyeView, allow_raw_pointers());
}
